...
def get_semi_data(seminum, num_flat, dataset, gen_list):
    if (num_flat-1) in gen_list:
        return True
    else:
        return False
    '''if dataset == 'ntu-xsub':
        if seminum == 5 and num_flat <= 33:
            return True
        elif seminum == 10 and num_flat <= 66:
            return True
        elif seminum == 20 and num_flat <= 132:
            return True
        elif seminum == 40 and num_flat <= 264:
            return True
        else:
            return False
    elif dataset == 'ntu-xview':
        if seminum == 5 and num_flat <= 31:
            return True
        elif seminum == 10 and num_flat <= 62:
            return True
        elif seminum == 20 and num_flat <= 124:
            return True
        elif seminum == 40 and num_flat <= 248:
            return True
        else:
            return False'''
...
class NTU_Generator():
    ...
    def gendata(self, phase):
        ...
        if phase == 'train':
            if self.semi == 5 and self.dataset == 'ntu-xsub':
                #gen_list = random.sample(range(32,550), 1)
                #gen_list = gen_list + [i for i in range(32)]
                gen_list = random.sample(range(550), 33)
                logging.info('gen_list:{} for semi:{}'.format(gen_list, self.semi))
        for folder, filename in sorted(self.file_list):
            ...
            if issample:
                ...
                if self.byol or phase == 'eval':
                    sample_paths.append(path)
                    sample_label.append(action_class - 1)  # to 0-indexed
                elif get_semi_data(self.semi, class_num_list[action_class - 1], self.dataset, gen_list):
                    sample_paths.append(path)
                    sample_label.append(action_class - 1)  # to 0-indexed
        
        '''if phase == 'train':
            print('train:',num_flat)
        else:
            print('eval:',num_flat)'''

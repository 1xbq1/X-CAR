...
def load_checkpoint(work_dir, model_name='resume'):
    ...
    elif model_name == 'load_supcon':
        file_name = '{}/checkpoint_sup.pth.tar'.format(work_dir)
...
def save_checkpoint(model, classifier, optimizer, scheduler, epoch, best_state, is_best, work_dir, save_dir, model_name):
    for key in model.keys():
        model[key] = model[key].cpu()
    for key in classifier.keys():
        classifier[key] = classifier[key].cpu()
    '''for key in model1.keys():
        model1[key] = model1[key].cpu()'''
    '''checkpoint = {
        'model':model, 'model1':model1, 'optimizer':optimizer, 'scheduler':scheduler,
        'best_state':best_state, 'epoch':epoch,
    }'''
    checkpoint = {
        'model':model, 'classifier':classifier, 'optimizer':optimizer, 'scheduler':scheduler,
        'best_state':best_state, 'epoch':epoch,
    }
...
def save_checkpoint_sup(model, optimizer, scheduler, epoch, work_dir, save_dir, model_name):
    for key in model.keys():
        model[key] = model[key].cpu()
    '''for key in model1.keys():
        model1[key] = model1[key].cpu()'''
    ...
    cp_name = '{}/checkpoint_sup.pth.tar'.format(work_dir)
    torch.save(checkpoint, cp_name)
